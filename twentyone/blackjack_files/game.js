...
//	stage.addChild(spr);
	...
	spr.onclick = function () {
		putButSound();
		window.location.href = 'http://www.duopao.com/game/commend';
	}
//	stage.addChild(spr);
}
...
function goDealer() {
	...
	var j = Math.random();
	if (j * 1000 <= chance * 10) {
		console.log('ИГРОК ВЫИГРАЕТ');
		var n = 1;
		while (n > 0) {
			console.log('goDealer 111');
			if (dscore <= 11) {
				console.log('Диллер ходит рандомом 111');
				var y = calcRandom();
				putOnTableDealer(y);
				if (checkPlayerWin() || checkDealerWin() || checkDrawn())
					return;
			} else if (dscore > 11 && dscore < 17) {
				console.log('Диллер ходит умно 111');
		...
		console.log('ИГРОК ПРОИГРАЕТ');
		var n = 1;
		while (n > 0) {
			if (dscore <= 11) {
				console.log('Диллер ходит рандомом 222');
				var y = calcRandom();
				putOnTableDealer(y);
				if (checkPlayerWin() || checkDealerWin() || checkDrawn())
					return;
			} else if (dscore > 11 && dscore < 17) {
				console.log('Диллер ходит умно 222');
...
function checkWinner(start) {
	...
	if (start) {
		console.log('я в чек виннер старт');
